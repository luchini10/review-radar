...
titles, source titles, IDs, headers, and secrets never enter the trace or the
...
Before collection, an exact candidate-owned title/URL record that passes the
unchanged shared asset-identity predicate continues immediately. Title-visible
brand, model, and complete-product type is preferred, and the existing
conservative direct-manufacturer or established-retailer product-slug path
remains intentional. When no exact record passes but at least one lacks title
metadata, the candidate is deferred to bounded page verification. Only all-
affirmative mismatches quarantine it; sibling slugs and unknown-retailer slug
authority still fail. Response metadata is preflight only, never verified
...
omitted. Historically, PR-3H rolled contract v8/runtime v7 and its next
sanitized Phase D evidence boundary to v5. PR-3J below supersedes those three
versions; research schema v5 and prompt v6 do not change.

PR-3J updates only the missing-metadata branch. The complete consulted-source
action supplied by the Responses API is URL-only; a citation annotation can
carry a title, but source-action titles are not guaranteed. Source preflight is
therefore tri-state: any available exact title that proves identity retains the
candidate; when none proves identity but any exact source lacks title metadata,
the candidate is deferred to the existing bounded DNS-pinned fetch and
unchanged page/entity verifier; only all-affirmative title mismatches quarantine
it. Deferral grants no evidence, eligibility, or shopper-visible claim.

The current exact-key diagnostic reports submitted, accepted/continued,
...
D records use sanitized evidence v6 so they can also carry the separately
sanitized tri-state source-filter counts.
...
8. **Bounded organic identity resolution** (`resolveSerperIdentityLeads`, `lib/search/serper.ts`): behind the default-off `REVIEW_RADAR_ORGANIC_IDENTITY_RESOLUTION` flag, model-specific structured Shopping identities first receive the existing guarded merchant-URL recovery at zero additional cost; only identities still lacking a safe page may receive one organic `"<identity> product page"` lookup. Compacted hard specifications cannot establish a resolver identity. Duplicate leads are prioritized by recurrence across distinct parent queries, then total recurrence, before the request-wide cap of 4. Every result still passes the existing organic normalizer, cheap prefilter, requested-type classifier, product-eligibility gate, and shared product-page identity selector. Flag-off mode dispatches no lookup and does not change the candidate pool.
9. **Final AI research** (`client.responses.create`, final model, `tools:[{type:"web_search"}]`, `tool_choice:"required"`, **strict JSON schema** `recommendationResultJsonSchema`, 180s timeout). The LLM returns structured candidates + summary prose. `selectFinalResearchCandidates` shortlists the strongest Serper candidates before this call, and `chooseFinalResearchContext` uses lighter web-search context when candidate coverage is already strong. `researchSystemPrompt`/`buildResearchPrompt` from `lib/researchPrompt.ts` pass the shortlisted candidates and market-coverage snapshot into the prompt.
10. **Parse + validate** the model output: `normalizeResearchResult` → `recommendationResultSchema.safeParse` (`lib/recommendationSchema.ts`, `lib/normalizeResearchResult.ts`).
11. **Merge candidate sets**: `serperCandidateToRecommendation` converts Serper candidates; `mergeProductRecommendations` merges them with the LLM's `candidate_products`.
12. **Citation verification**: `collectVerifiedSourceUrls(response)` + Serper URLs (trusted without re-fetch). If none, `collectReachableCitationUrls` does a live reachability check. `filterResultToVerifiedCitations` drops products lacking a verified citation.
13. **Result-issue gate** (`getRecommendationResultIssue`): triggers fallbacks (see §9).
14. **Requirement filtering** (`filterResultByRequirements`, `lib/requirementValidation.ts`).
15. **Enrichment chain**: `enrichResultWithReviewEvidence` (trust-ladder) → `enrichProductAssets` (images/price/metadata) → `verifyMissingRequirementEvidence` (rescue unknown facts) → `revalidateResultCandidates`. `buildAdaptiveVerificationBudget` keeps expensive enrichment/rescue focused on likely visible products, defaults to 3-product concurrency, and checks the most important missing facts first. The trust ladder now receives the generated `buyingRubric` internally so it can search for category-specific facts, quality signals, owner-review themes, and red flags before ranking.
16. **Source-quality upgrade** (`upgradeWeakSourceEvidence`, `lib/requirementEvidenceRescue.ts`): runs AFTER `revalidateResultCandidates` and BEFORE scoring. For each candidate that (a) has a model-number token (strong product identity), (b) passes all hard requirements, and (c) has no verified price, no owner rating, and no tier-1 editorial or tier-2 marketplace citation from a different host (`hasUsefulCommerceEvidence` using `sourceTier.ts`), a single `searchSerperShopping` call looks for the same product on a better-sourced page. Before model/token overlap can accept a candidate, `looksLikeSameProduct` rejects explicit product-type conflicts through `classifyProductTypeMatch`; it also rejects an explicit different token in the same model family. Exact model evidence, safe merchant model paths, and sparse candidates without an explicit conflict retain the existing identity path. After the identity gate passes, price, rating, reviewCount, citation, and image are merged via existing rescue helpers. Cap: `MAX_SOURCE_UPGRADE_CANDIDATES = 3`. Debug visibility: `debug.stageFunnel.sourceUpgradeTraces` — one `SourceUpgradeTrace` per attempt recording `name`, `query`, `candidatesReturned`, `candidatesEvaluated`, `noMatchReason` (`"shopping_results_empty"` / `"identity_rejected"` / `"no_attachable_fields"`), `candidateSample` (≤5 shopping results with identity match result and rejection reason), `evidenceAttached`, and `attachedFields`.
17. **No-exact sanity fallback** (`shouldRunNoExactSanityFallback`) for common searches.
18. **Score & select** (`scoreAndSelectRecommendations`, `lib/recommendationScoring.ts`).
19. **Optional narration** (`REVIEW_RADAR_LLM_NARRATION=on`): a cheap explainer LLM pass that rewrites only the displayed cards' prose (`lib/finalSynthesis.ts`), guarded so it cannot change the set, prices, specs, or citations. It is off by default for normal searches because it improves wording, not product accuracy.
20. **Finalize**: `prioritizeProductPageUrlsInResult` + `removeUserHiddenResultFields` (strips `scoreBreakdown`; hides `specConstraints` unless debug). Respond `{ result }` (+`debug` when `x-reviewradar-debug: true` and non-prod).
...
- **Bounded organic identity resolution** (`resolveSerperIdentityLeads`): structured Shopping product IDs can provide an internal lead only when the title has a non-spec model code, a source-leading brand identity, requested-type exactness, and product eligibility. When its flag is on, safe merchant recovery runs first; remaining leads are aggregated and recurrence-ranked, then limited to four organic lookups. Only pages satisfying the shared identity and safety gates enter the pool. Generic and ambiguous short-code identities stay unresolved; the resolver never retries through Shopping.
