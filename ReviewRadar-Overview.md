...
8. **Bounded organic identity resolution** (`resolveSerperIdentityLeads`, `lib/search/serper.ts`): behind the default-off `REVIEW_RADAR_ORGANIC_IDENTITY_RESOLUTION` flag, model-specific structured Shopping identities that still lack a safe product page after merchant-URL recovery can receive one organic `"<identity> product page"` lookup. The request-wide cap is 4 unique identities. Every result still passes the existing organic normalizer, cheap prefilter, requested-type classifier, product-eligibility gate, and shared product-page identity selector. Flag-off mode dispatches no lookup and does not change the candidate pool.
...
- **Bounded organic identity resolution** (`resolveSerperIdentityLeads`): structured Shopping product IDs can provide an internal lead only when the title is model-specific, requested-type exact, and product-eligible. When its flag is on, the resolver deduplicates those leads, limits the request to four organic lookups, and admits only pages that satisfy the shared identity and safety gates. Generic identities stay unresolved; the resolver never retries through Shopping.
