...
Research 8 to 15 distinct complete products. Include leading, value, and constraint-relevant options. Do not use benchmark answers or hardcoded product lists. Return the exact brand, exact model, and concise complete-product product_type as separate identity fields; do not mix sibling-model or adjacent-product identities. Do not emit product_name; ReviewRadar constructs the internal product name deterministically from brand, model, and product_type after validation. Exclude accessories, replacement parts, editorial pages, category pages, used products, and mismatched variants as candidates. Preserve discovery order in the candidate array. ReviewRadar assigns internal candidate and fact IDs after validation; do not emit synthetic identifiers.

Return exactly two logically distinct source_urls for every candidate, both copied exactly from this response's hosted web-search sources. Tracking-only or fragment-only variants of one physical page do not count as distinct. At least one source must be a direct manufacturer or established retailer complete-product page whose exact title and exact URL together prove the exact brand, model, and complete product_type under ReviewRadar's bounded identity and product-page rules. A safe manufacturer or established-retailer product URL slug may carry the exact model when the title still proves the brand and complete product type. Editorial, review, comparison, support, category, search, and listing pages do not satisfy this complete-product-page requirement. Use the other source for independent requirement or fact evidence. Exclude any candidate without an identity-safe complete-product page among its two exact response-owned sources. Do not invent, normalize, shorten, reconstruct, or repeat a candidate source URL. Preserve each candidate's source order. Use zero-based source_indexes in every requirement and fact lead to refer only to source_urls within the enclosing candidate; never repeat a URL in a lead and do not repeat an index or reference. Every index must be either 0 or 1. Do not repeat raw URLs inside leads.
