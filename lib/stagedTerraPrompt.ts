...
Return exactly two logically distinct source_urls for every candidate, both copied exactly from this response's hosted web-search sources. Tracking-only or fragment-only variants of one physical page do not count as distinct. At least one response-owned hosted-search source must prove the exact brand, model, and complete product_type when its exact title and exact URL are considered together under ReviewRadar's bounded identity rules. Prefer a title that visibly identifies all three fields. A direct manufacturer or established retailer product page may instead carry the exact model in its safe product URL slug when the title and URL together still prove the complete-product identity. Use the other source for independent requirement or fact evidence. Exclude any candidate if neither of its exact response-owned title-and-URL pairs proves its exact identity. Do not invent, normalize, shorten, reconstruct, or repeat a candidate source URL. Preserve each candidate's source order. Use zero-based source_indexes in every requirement and fact lead to refer only to source_urls within the enclosing candidate; never repeat a URL in a lead and do not repeat an index or reference. Every index must be either 0 or 1. Do not repeat raw URLs inside leads.
