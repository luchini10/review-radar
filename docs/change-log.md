...
- Kept every direct dependency specification unchanged and refreshed only the
  vulnerable lock resolutions, required companions, and Next's matching ESLint
  configuration. Next now resolves to 16.3.3 and Sharp to 0.35.4.
- Added a lock-only regression that prevents every RR-109 package from falling
  below the proven clean versions and keeps Next aligned with its lint config.
- Retained the agent-rule block generated by Next 16.3.3 so future framework
  runs do not recreate a tracked change.

#### Verified

- The exact corrected lock passes a credential-isolated public npm 11.12.1
  audit with zero vulnerabilities. Sixty-one changed registry artifacts match
  exact public tarball URLs and integrity values; package-only npm reports no
  problems.
- The regression fails against the exact base lock and passes 4/4 after the
  correction. Clean Windows install/native loading, full tests 1,699/1,699,
  typecheck, Next 16.3.3 build, Playwright 17/17, lint with zero errors/three
  old warnings, and all five controller partitions pass.
- Actual Linux execution remains unverified because no Linux runtime is
  available. RR-109 is Fixed, while ReviewRadar remains `NOT READY` because
  current live quality and hosted operational proof are still missing.
- Independent exact review returned `VERIFIED`, no material correction,
  ...
  61-artifact provenance, focused 6/6, range semantics, issue arithmetic, and
  generated Next block authentication.
...
- Added a bounded public review packet to the runner terminal so required
  per-artifact audits do not require ordinary access to protected fixtures.
...
- Corrected launcher/runner/readiness wall: 71/71.
