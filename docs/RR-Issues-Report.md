...
**Post-R3 RR-061 resolution (2026-07-11):** Fixed deterministically. Static path analysis establishes the R2 winners as extracted `page_image` candidates: initial `existing` candidates carry empty evidence and no verified context, while every extracted page image inherited page-wide identity and could therefore pass at Medium without image evidence or High when its attributes echoed the target. The serialized `retailer_page` field alone is ambiguous because it represents both sources, and the saved ledger contains no fetched HTML; it cannot prove that OG or Product JSON-LD images existed at runtime. The resolver now keeps page verification on page-bound metadata but never grants it wholesale to `<img>` elements. Page images must match the target through their own attributes or path.

The filename backstop now recognizes adjacent alphabetic-family plus numeric tokens in either order and repeated non-generic alphabetic family tokens. Generic image/color/view/package words, retailer/source words, brand/category words, CDN modifiers, dimensions, units, versions, file counters, and opaque hashes remain excluded. The rule is dictionary-free and request-independent. It rejects both QRevo captures, `Saros_20` on Q7 Max+, the earlier `Saros_Z70` mismatch, and a different split generation such as Saros 20 on Saros 10. It preserves family-adjacent sizes such as `ipad_11` when the target does not assert a different numbered family, same-model files, neutral files, matching Product JSON-LD, Google thumbnails, Amazon modifiers, and the prior navigation guards.
...
`https://eustore.hermanmiller.com/products/embody-gaming-chair`. The
manufacturer host and shared descriptive family name passed identity
verification even though the URL explicitly identifies the different Gaming
variant. This is the Direct-Terra equivalent of a wrong-model destination.

**Where it occurs:** `lib/directTerraAssetVerifier.ts` and the downstream
website-selection path. Exact-model protections are strong for alphanumeric
model codes, but descriptive-name targets do not currently treat a
candidate-only variant/type modifier as a conflict.

**Evidence:** Untracked fixture
`tests/fixtures/review-radar-live/oai-t8d-root-cause-diagnostic-437a682/eval-broad-office-chair.run2.json`,
rank 5. The final product name is `Herman Miller Embody Chair`; the accepted
manufacturer destination path is `/products/embody-gaming-chair`.

**Expected:** A standard/base descriptive product must not receive a link or
image whose candidate title/path adds an identity-changing variant. The rule
must be category-independent and preserve harmless retailer wording.

**Suggested fix or next action:** Add a fixture-derived fail-first test and
apply the existing product-type/variant conflict evidence as a veto for
descriptive-name identities. Prove the same primitive on at least one unrelated
...
grill `with ... Charcoal Tray`, and a brushless drill/driver bundled with an
impact driver. The scorer searches the complete product name for a prohibited
substring, so secondary functionality and included products become false
hard-type violations.

**Where it occurs:** `lib/directTerraEvaluation.ts` in the
`wrongTypeTerms` substring loop.

**Evidence:** The two unrelated-category fixtures are
`eval-con-gas-grill-600-4burner.run2.json` (term `charcoal`) and
`eval-con-cordless-drill-150-brushless.run2.json` (term `impact driver`) under
the untracked OAI-T8D evidence directory. Both reports separately mark every
requested hard feature and budget as satisfied.

**Expected:** A wrong-type term is a failure only when it identifies the
recommended product's primary type or contradicts a hard requirement. It must
not fail a valid requested product merely because the name discloses an
additional mode or bundled item.

**Suggested fix or next action:** Version, rather than silently rewrite, the
frozen evaluator. Add cross-category controls for genuine wrong products,
...
benchmark brand into two tokens (`char broil`) while the product spelling
becomes one (`charbroil`). Recall and first-loss attribution therefore report
the leader absent even though Terra ranked it.

**Where it occurs:** `scripts/goldBenchmark.mjs` in `coversLeader` and every
evaluation/diagnostic consumer of that contract.

**Evidence:** Untracked fixture
`eval-con-gas-grill-600-4burner.run2.json` under the OAI-T8D directory. Rank 2
is `Charbroil Performance Series ...`; the saved score still lists
`char-broil` as missed and the first-loss report calls its source evidence
absent.

**Expected:** Harmless punctuation/spacing differences in a brand or model
must compare equivalently, while token boundaries must still prevent substring
matches such as `Q7` versus `Q70`.

**Suggested fix or next action:** Add a generalized compact-token equivalence
...
aggregate reason counts. It does not retain bounded normalized candidate
identities, so it cannot prove whether the correct result was present and
rejected or whether every mapped result described another product. Likewise,
the recommendation trace calls every missed leader absent from source evidence
even when Terra's own Close Matches section explicitly discusses the leader
(for example Roborock Q10 S5+ and eufy Auto-Empty C10).

**Where it occurs:** `lib/directTerraFirstLoss.ts` and the optional diagnostic
callbacks in the citation/organic/Shopping adapters.

**Evidence:** All eight untracked OAI-T8D fixtures. The aggregate contains 31
fully accounted ranked products, seven missing links labeled identity
rejection, and zero `terra_source_evidence_present_not_ranked` leader outcomes.
`eval-con-robot-vac-300-selfempty.run1.json` explicitly discusses two frozen
leaders under `Close matches, but not ranked`, contradicting that zero count.

**Expected:** The diagnostic must retain enough sanitized identity evidence to
separate correct-result absence, normalization loss, false identity rejection,
safe rejection, and later selection loss without retaining secrets, provider
IDs, or full URLs.

**Suggested fix or next action:** Before another live window or behavior
repair, add bounded server-only candidate identity samples (normalized
brand/model/type evidence, host class, path identity evidence, and verdict
...
| **Phase** | OAI-T8D cross-category first-loss diagnostic |
| **Severity** | High |
| **Title** | Bare Direct-Terra ranked labels silently erase parsed product cards and prices |
| **Status** | Fixed |

**Description:** The verifier-v4 gas-grill diagnostic returned a complete Terra
report with three ranked products, but Terra used bare labels such as
`#1 Best Match — Monument Grills Mesa M415BZ` (no space after `#`). Markdown
renders those lines as paragraphs, not H1 headings. ReviewRadar's independent
ranked-product and price parsers accepted only Markdown H2 through H4. They
therefore returned zero ranked products and prices, created zero asset targets,
dispatched zero asset-provider requests, rendered no shortlist cards, and
incorrectly scored recall as `0/4`.

**Where it occurs:** `lib/directTerraReportOutline.ts` in
`extractDirectTerraPicks`, `lib/directTerraEvaluation.ts` in
`parseRankedProducts`, `lib/directTerraPriceEstimate.ts` in ranked-section
binding, and anchor handling in `components/DirectTerraReport.tsx`.

**Evidence:** Untracked fixture
`tests/fixtures/review-radar-live/oai-t8d-root-cause-diagnostic-9e041e0/eval-con-gas-grill-600-4burner.run1.json`.
The saved report contains three valid U+2014 bare ranked labels and complete
research beneath each. The saved score and first-loss trace contain zero ranked
products and zero asset targets. Encoding was ruled out; the missing bare-label
contract is the first loss.

**Expected:** A semantically exact `#N Best Match` heading should be parsed
consistently for the observed bare-label form and Markdown H1 through H4. A
ranked section must end at the next heading of equal or higher structural depth
or a required non-product report section so comparison/advice text cannot bleed
into the last product. Shortlist, evaluation, price binding, anchors, and
rendering must use the same contract.

**Resolution (2026-07-23, commit `59ae9bc`):** One shared ranked-section parser
now recognizes bare labels and H1-H4, preserves the first duplicate rank, and
closes sections at structurally safe headings and required report boundaries.
Shortlist extraction, evaluation, price binding, and UI anchors consume that
contract. Fail-first coverage reproduced the saved gas form; focused parser,
evaluation, response, price, and UI tests pass 46/46. The current zero-live
replay extracts all three gas products and accounts for 12/12 ranked products
across the three completed live fixtures. The sanitized gas fixture did not
...
**Resolution (2026-07-23, commit `95fddb5`):** The verifier now ignores ancestor
taxonomy before the first descriptive model token and accepts later descriptive
detail only when the locked target/category explains it or every non-neutral
detail is independently present in the candidate title. Opaque commerce IDs
are recognized only in explicit ID path positions. Exact manufacturer or
popular-retailer model slugs may resolve a generic eligibility `unknown`, but
never an explicit negative. Cross-category tests cover Steelcase, Herman
Miller, Haworth, Branch, and a refrigerator retailer page. Partial
corroboration cannot hide an undisclosed suffix. The complete RR-093 sibling,
