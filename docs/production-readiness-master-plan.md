...
| PR-021 | P1 | confirmed; correction selected | The independently authorized PR-4B attempt-1 launcher exited 1 after about 9.55 seconds with one generic sentence. The prospective output leaf remained absent, proving the trusted runner never reached its pre-provider directory-creation point, but the exact failure stage is unknowable without inspecting credentials or retrying. | The one-shot launcher collapses every process, repository/trust, approval, credential, post-credential reauthentication, child-construction, spawn-error, and signaled-child failure into one generic catch message. | Emit one closed, versioned, nonsecret terminal object with typed stages. Never include raw errors, paths, credential presence/values/lengths/hashes/prefixes, or arbitrary text. Add deterministic injection coverage for every stage and preserve exact no-retry/one-shot behavior. | Fail-first expectations against the current generic message; exhaustive stage mapping; raw-error and credential-canary negatives; exact single-terminal stdout/stderr contract; focused/full/static/build/E2E/controller walls; independent source and exact-commit review. | Attempt 1 remains consumed and no provider request occurred. The correction cannot authorize a retry, replacement, attempt 2, flag change, deployment, or release. Manual credential inspection is prohibited; exact past cause may remain unknowable. |
...
- Current gate: preserve the consumed stop, correct PR-021 with typed nonsecret
  launcher terminal stages and exhaustive regressions, and independently review
  the correction. Any future live continuation requires a separate protocol and
  governance decision; this phase authorizes none.
...
- the PR-4A truth expires 2026-09-12, candidate-pool Jaccard is intentionally
  ...
  origin/fixture/manual review. PR-021 launcher observability is open, and the
  legacy scorecard remains non-authorizing;
