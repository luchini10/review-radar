...
| RR-061 | Non-image/irrelevant assets became product images | Medium | Needs Investigation | Deterministic gap found live | `productImageResolver.test.mjs`; `productAssets.test.mjs`; `sourceQualityUpgrade.test.mjs`; post-RR-061 B1 fixture | High | Navigation/flyout and host-identity protections remain green, but verified page context can still override an explicit conflicting model in the image path (Q5 Max+ card using Saros Z70 image). |
...
| RR-070 | Unrelated provider brand metadata contaminated source-upgrade query | Medium | Needs Investigation | Live diagnostic gap | post-RR-061 B1 fixture; source-upgrade trace | Medium | ILIFE A12 Pro received `Bose` metadata brand and query `Bose ILIFE A12 Pro`; no evidence attached. Requires deterministic reproduction before a later fix. |
