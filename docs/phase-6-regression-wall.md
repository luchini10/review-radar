...
| RR-061 | Non-image/irrelevant assets became product images | Medium | Fixed | Deterministic protection | `productImageResolver.test.mjs`; `productAssets.test.mjs`; `sourceQualityUpgrade.test.mjs`; `discoveryImprovements.test.mjs` | High | Reopened live and re-fixed. Invalid pages, logos, placeholders, trackers, navigation/flyout/menu/layout assets, and retailer-host identity pollution are rejected while contextual Amazon/CDN images survive. |
