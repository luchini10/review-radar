...
describe("OAI-2A runner offline boundary", () => {
  it("preflights the isolated v2 case without an API key or evidence write", async () => {
    const env = { ...process.env };
    delete env.OPENAI_API_KEY;
    const { stdout, stderr } = await execFileAsync(
      process.execPath,
      ["--no-warnings", "scripts/run-oai-2a.mjs", "--primary-01-v2-smoke", "--preflight-only"],
      { cwd: process.cwd(), env },
    );
    assert.equal(stderr, "");
    const output = JSON.parse(stdout);
    assert.equal(output.status, "preflight_passed");
    assert.equal(output.runMode, "primary-01-v2-smoke");
